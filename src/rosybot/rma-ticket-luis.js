...
// State
const { RmaTicketLuisState } = require('./rma-ticket-luis-state');
...
class RmaTicketLuis {
    ...
    constructor(convoState, userState, botConfig) {
        ...
        this.state = new RmaTicketLuisState(convoState, userState);
    ...
    async onTurn(turnContext) {
        ...
        case NONE_INTENT:
        default:
            await turnContext.sendActivity(`RmaTicketLuis dialog cannot fulfill this request.`);
        }
    }
    /**
     *
    ...
    async handleDeviceUpdate(homeAutoResults, context) {
        ...
        if (operations === undefined) {
            await context.sendActivity(`You reached the "RmaTicketLuis" dialog but I am still under development...`);
        }
        else {
            // Update device state.
            ...
            await context.sendActivity(`You reached the "RmaTicketLuis" dialog.`);
...
module.exports.RmaTicketLuis = RmaTicketLuis;
